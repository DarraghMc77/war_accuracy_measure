...
class DetectionMAP:
    def __init__(self, n_class, pr_samples=11, overlap_threshold=0.5, totalIoU=0):
        """
    ...
    def evaluate(self, pred_bb, pred_classes, pred_conf, gt_bb, gt_classes):
        ...
        if gt_bb.ndim == 1:
            gt_bb = np.repeat(gt_bb[:, np.newaxis], 4, axis=1)
    ...
    def compute_false_positive(pred_cls, pred_conf, conf_threshold, gt_cls, IoU, class_index):
        # ** check if this works
        gt_size = gt_cls.shape[0]
        if gt_size == 0:
            return np.sum(pred_cls == class_index)
